<x-website-layout>
    ...
    <script>
        ...
        function populateFilters(filters) {
            ...
            filters.categories.forEach(category => {
                ...
                <div class="custom-checkbox">
                    ...
                    <label for="category-${category.id}" class="text-gray-700 dark:text-gray-300 hover:text-primary-600">${category.name}</label>
                </div>
            `;
            });
            ...
            filters.brands.forEach(brand => {
                ...
                <div class="custom-checkbox">
                    ...
                    <label for="brand-${brand.id}" class="text-gray-700 dark:text-gray-300 hover:text-primary-600">${brand.name}</label>
                </div>
            `;
            });
        ...
        function initEventListeners() {
            ...
            // Category filters - use event delegation for dynamically created elements
            $(document).on('change', '.filter-category', function() {
                applyFilters();
            });

            // Brand filters - use event delegation for dynamically created elements
        ...
        function applyFilters() {
            ...
            $('.filter-category:checked').each(function() {
                selectedCategories.push(parseInt($(this).val())); // Convert to integer
            });

            // Get selected brands
            const selectedBrands = [];
            $('.filter-brand:checked').each(function() {
                selectedBrands.push(parseInt($(this).val())); // Convert to integer
            });
            ...
            filteredProducts = allProducts.filter(product => {
                ...
                // Category filter - check if any selected category matches
                if (selectedCategories.length > 0 && 
                    !selectedCategories.includes(parseInt(product.category_id))) {
                    return false;
                }

                // Brand filter - check if any selected brand matches
        ...
        function renderProductGrid(product) {
            ...
            const minPrice = parseFloat(product.price.min) || 0;
            const maxPrice = parseFloat(product.price.max) || 0;
            ...
            if (product.has_multiple_prices) {
                ...
                priceHtml = `
                        ${product.discount > 0 ? `
                            <span class="text-lg font-bold text-primary-600 dark:text-primary-400">Rs. ${(minPrice * (1 - product.discount/100)).toLocaleString()}</span>
                            <span class="text-sm text-gray-500 line-through ml-2">Rs. ${minPrice.toLocaleString()}</span>
                        ` : `
                            <span class="text-2xl font-bold text-primary-600 dark:text-primary-400">Rs. ${minPrice.toLocaleString()}</span>
                        `}
                    `;
            }
            ...
            window.assetBaseUrl = "{{ asset('backend') }}";
            window.noImagePath = "{{ asset('backend/assets/images/no_image.png') }}";

            // Decode image_paths JSON safely
            let imageUrl = window.noImagePath; // fallback
            if (product.image_paths) {
                imageUrl = product.image_paths;
            }
            ...
            return `
                    <div class="bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-slow product-card">
                        <div class="relative product-image-container">
                            <img src="${imageUrl}" alt="${product.name || 'no img'}" class="w-full h-48 object-cover">
        ...
        function renderProductList(product) {
            // Convert price values to numbers and handle null/undefined cases
            const minPrice = parseFloat(product.price.min) || 0;
            const maxPrice = parseFloat(product.price.max) || 0;
            ...
            if (product.has_multiple_prices) {
                ...
                priceHtml = `
                    ${product.discount > 0 ? `
                        <span class="text-2xl font-bold text-primary-600 dark:text-primary-400">Rs. ${(minPrice * (1 - product.discount/100)).toLocaleString()}</span>
                        <span class="text-lg text-gray-500 line-through ml-2">Rs. ${minPrice.toLocaleString()}</span>
                    ` : `
                        <span class="text-2xl font-bold text-primary-600 dark:text-primary-400">Rs. ${minPrice.toLocaleString()}</span>
                    `}
                `;
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-slow product-card col-span-1 sm:col-span-2 lg:col-span-3">
                    <div class="flex flex-col md:flex-row">
                        ...
                        <div class="md:w-2/3 p-6">
                            <h3 class="text-xl font-bold mb-2 dark:text-white">${product.name}</h3>
                            <p class="text-gray-600 dark:text-gray-300 mb-4">${product.short_description??"no description found"}</p>
                        </div>
                    </div>
                </div>
            `;
